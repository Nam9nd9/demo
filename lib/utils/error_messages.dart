final Map<String, String> errorMessages = {
  ...
  "CUSTOMER_KHACH_TRANG_GROUP_REQUIRED":
      "Khách hàng tên 'Khách Trắng' chỉ có thể được tạo vào nhóm 'Khách Trắng'.",
  "PHONE_NUMBER_REQUIRED": "Số điện thoại đã được sử dụng.",
  "ONLY_ALLOW_CREATE_CUSTOMER_NAME_KHACH_TRANG_IN_GROUP_KHACH_TRANG":
      "Chỉ có thể tạo khách hàng tên 'Khách Trắng' vào nhóm 'Khách Trắng'.",
  ...
  "CAN_NOT_UPDATE_ITEMS_WHEN_IMPORT_BILL_STATUS_IS_NOT_COMPLETED_OR_CANCELLED":
      "Không thể cập nhật sản phẩm khi trạng thái phiếu nhập không phải 'Đã hoàn thành' hoặc 'Đã hủy'.",
  "INSPECTION_REPORT_ALREADY_EXISTS": "Mã phiếu nhập đã kiếm.",
  "CAN_NOT_UPDATE_INSPECTION_REPORT_WHEN_STATUS_IS_NOT_INSPECTING":
      "Không thể cập nhật báo cáo kiểm tra khi trạng thái không phải 'Đang kiểm'.",
  ...
  "CAN_NOT_CANCEL_WHEN_INVOICE_STATUS_IS_NOT_READY":
      "Không thể hủy đơn khi đơn hàng không ở trạng thái 'Sẵn sàng lấy hàng'.",
  ...
  "CAN_NOT_CANCEL_WHEN_DELIVERY_STATUS_IS_NOT_ready_to_pick_OR_picking_OR_money_collect_picking":
      "Không thể huỷ đơn khi trạng thái không phải 'Chờ lấy hàng'",
  "CAN_NOT_UPDATE_INSPECTION_REPORT_WHEN_STATUS_IS_NOT_CHECKING":
      "Không thể cập nhật phiếu kiểm khi trạng thái không phải 'Đang kiểm'",
  "CAN_NOT_UPDATE_ITEMS_WHEN_IMPORT_BILL_COMPLETED":
      "Không thể cập nhật khi phiếu nhập đã hoàn thành",
  "CAN_NOT_UPDATE_WHEN_INVOICE_STATUS_IS_NOT_ready_to_pick":
      "Không thể cập nhật đơn hàng nếu đơn hàng ở trạng thái không phải 'Chờ lấy hàng'",
...
String getDynamicErrorMessage(String errorCode) {
  final productMatch = RegExp(r"^PRODUCT_'(.+)'_NOT_ENOUGH_IN_(.+)\.?$").firstMatch(errorCode);
  if (productMatch != null) {
    final productName = productMatch.group(1)!;
    final warehouse = productMatch.group(2)!;
    return "$productName không đủ hàng trong kho '$warehouse'";
  }

  final oldFormatMatch = RegExp(r"^PRODUCT_(.+)_NOT_ENOUGH_IN_(.+)$").firstMatch(errorCode);
  if (oldFormatMatch != null) {
    final product = oldFormatMatch.group(1);
    final branch = oldFormatMatch.group(2);
    return "$product không đủ hàng trong kho '$branch'";
  }
...
String getErrorMessage(String errorCode) {
  ...
  return errorMessages[code] ?? getDynamicErrorMessage(code);
}
